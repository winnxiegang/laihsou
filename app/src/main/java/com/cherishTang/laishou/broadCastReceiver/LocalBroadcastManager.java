...
public final class LocalBroadcastManager {
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                LogUtil.show( "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            ...
            if (entries != null) {
                if (debug) {
                    LogUtil.show("Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        LogUtil.show( "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            LogUtil.show( "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "LocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                LogUtil.show("  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            LogUtil.show(  "  Filter did not match: " + reason);
                        }
                    }
                }
