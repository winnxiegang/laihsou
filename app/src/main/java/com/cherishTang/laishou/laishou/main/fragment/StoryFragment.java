...
import com.cherishTang.laishou.laishou.main.adapter.StoryAdapter;
...
public class StoryFragment extends BaseRecyclerViewFragment<ArticleBean> {
    ...
    protected void requestData() {
        super.requestData();
        ApiHttpClient.getArticlePage(new Gson().toJson(new MainLaiShowListBean(mCurrentPage, 1, UserAccountHelper.getLocalSubstation().getId(),mPage)),this,stringCallback);
    }
    ...
    public void onItemClick(View view, int position) {
        ...
        try {
            ...
            Bundle bundle = new Bundle();
            bundle.putString("videoCoverImage",adapter.getList().get(position).getArticleTitle());
            bundle.putString("path", adapter.getList().get(position).getImg());
            bundle.putString("title",adapter.getList().get(position).getArticleTitle());
...
//        LaiShowWebViewActivity.show(getActivity(), bundle);
    ...
    protected BaseRecyclerViewAdapter<ArticleBean> getRecyclerAdapter() {
        return new StoryAdapter(getActivity());
    }
